...
import json
import subprocess
from config import JSON_PATH, RULES_V4_PATH, LOGFILE
...
def apply_firewall_rules():
    ...
    try:
        ...
        if process.returncode != 0:
            ...
            log_msg(f"[ERROR] iptables-restore failed: {err_decoded}")
            with open(LOGFILE, "a") as f:
                f.write(f"IPTABLES ERROR:\n{err_decoded}\n")
        else:
            save_iptables_rules()
            flush_conntrack()
        ...
        log_error("Applying iptables rules", e)
...
def main():
    ...
    if os.path.exists(RULES_V4_PATH):
        try:
            ...
            log_error("Startup rule restoration", e)
